...
def min_accuracy(args: argparse.Namespace) -> float:
    ...
    """
    if args.dataset.endswith(("pathmnist", "pathmnist-c")) and args.train_subset == 1.0:
        return 0.85
...
def test(
    ...
    for metric_name, metric_value in sorted(best_test_metrics.items()):
        if isinstance(metric_value, float):
            logger.info(f"Test metric {metric_name}: {metric_value:.6g}")
...
def main() -> None:
    ...
    if not hasattr(model, "default_cfg") or model.default_cfg is None:
        model.default_cfg = {}
    model.default_cfg["input_size"] = data_config["input_size"]
    ...
    model = wrap_model(
        ...
        prior_precision_grid_size=args.prior_precision_grid_size,
        log_prior_precision_min=args.log_prior_precision_min,
        log_prior_precision_max=args.log_prior_precision_max,
    ...
    has_trainable_parameters = any(param.requires_grad for param in model.parameters())
    if args.epochs == 0 and not has_trainable_parameters:
        optimizer = None
    else:
    ...
    if optimizer is None:
        lr_scheduler, num_epochs = None, 0
    else:
...
def create_datasets(
    ...
    train_dataset = create_dataset(
        ...
        label_noise_num_classes=args.num_classes,
        label_noise_seed=args.label_noise_seed,
        label_noise_mode=args.label_noise_mode,
    )
    if args.label_noise_fraction > 0.0:
        logger.info(
            "Applied %s label noise to %.1f%% of the training dataset "
            "(seed=%s).",
            args.label_noise_mode,
            100 * args.label_noise_fraction,
            args.seed if args.label_noise_seed is None else args.label_noise_seed,
        )
    ...
    varied_eval_severity = 2
    if args.dataset_id == "hard/pathmnist-c" and args.severities == (1,):
        varied_eval_severity = 1

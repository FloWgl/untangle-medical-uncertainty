...
def evaluate(
    ...
    if is_upstream_dataset and is_test_dataset:
        ...
        if path_indices.exists():
            indices = torch.load(
                path_indices, map_location=storage_device, weights_only=True
            )
        else:
...
def calc_correctnesses(
    ...
    dual_bma_topk = min(5, log_probs["log_dual_bmas"].shape[1])
    _, predicted_labels_dual_bma_top5 = torch.topk(
        log_probs["log_dual_bmas"], dual_bma_topk, dim=1
    )
    ...
    bma_topk = min(5, log_probs["log_bmas"].shape[1])
    _, predicted_labels_bma_top5 = torch.topk(
        log_probs["log_bmas"], bma_topk, dim=1
    )
    expanded_gt_hard_labels_original_bma = (
        targets["gt_hard_labels_original"]
        .unsqueeze(dim=1)
        .expand_as(predicted_labels_bma_top5)
    )
    targets["gt_hard_bma_correctnesses_original_top5"] = (
        predicted_labels_bma_top5.eq(expanded_gt_hard_labels_original_bma)
        .max(dim=1)[0]
        .int()
    )
    ...
    expanded_gt_hard_labels_bma = (
        targets["gt_hard_labels"].unsqueeze(dim=1).expand_as(predicted_labels_bma_top5)
    )
    targets["gt_hard_bma_correctnesses_top5"] = (
        predicted_labels_bma_top5.eq(expanded_gt_hard_labels_bma).max(dim=1)[0].int()
    )
...
def get_bundle(
    ...
    gt_biases_bregman_dual_bma = None
    gt_biases_bregman_bma = None
    gt_epistemics_bregman = None
    gt_predictives_bregman_dual_bma = None
    gt_predictives_bregman_bma = None
    gt_total_predictives_bregman_dual_bma = None
    gt_total_predictives_bregman_bma = None

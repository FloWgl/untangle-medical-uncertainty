#!/usr/bin/env bash
# Retry CIFAR-10 correctness prediction with the recovered W&B cdc24149 settings.
set -u

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT" || exit 1

RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)-cifar10-cp-cdc24149-retry}"
LOG_DIR="$REPO_ROOT/logs/cifar10_correctness_prediction_exact_${RUN_ID}"
...
exec 9>"$LOCK_FILE"
flock -n 9 || {
  echo "A CIFAR-10 correctness-prediction exact retry is already running for ${RUN_ID}."
  exit 1
}
...
COMMON_ARGS=(
  ...
  --ood-transforms-eval gaussian_noise,shot_noise,impulse_noise,defocus_blur,frosted_glass_blur,motion_blur,zoom_blur,snow,frost,fog,brightness,contrast,elastic,pixelate,jpeg
  --ood-transforms-test gaussian_noise,shot_noise,impulse_noise,defocus_blur,frosted_glass_blur,motion_blur,zoom_blur,snow,frost,fog,brightness,contrast,elastic,pixelate,jpeg

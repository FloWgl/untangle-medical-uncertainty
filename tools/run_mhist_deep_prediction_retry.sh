#!/usr/bin/env bash
# Retry MHIST deep prediction-head methods after the model input-size fix.
set -u

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT" || exit 1

RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)-mhist-deep-prediction-retry}"
LOG_DIR="$REPO_ROOT/logs/mhist_deep_prediction_retry_${RUN_ID}"
...
exec 9>"$LOCK_FILE"
flock -n 9 || {
  echo "An MHIST deep-prediction retry queue is already running for ${RUN_ID}."
  exit 1
}
...
echo "[$(date --iso-8601=seconds)] MHIST deep prediction retry ${RUN_ID}"
echo "Data: ${DATA_DIR}"

...
This fork extends the benchmark with reproducible CIFAR-10, PathMNIST,
PathMNIST-C, and MHIST experiments for medical-image uncertainty
disentanglement. See [REPRODUCIBILITY.md](REPRODUCIBILITY.md) for dataset setup,
experiment phases, HPC submission, analysis commands, and the repository's
artifact policy.

The medical experiment extensions and reproducibility package are maintained
by Florian Weigl. The original benchmark authors and citation are given below.
...
PathMNIST and PathMNIST-C are supported through the hard-label namespace by
default: use `hard/pathmnist` for clean data and `hard/pathmnist-c` for the
corrupted variants. Soft-label metrics and experiments are only available for
datasets that actually ship soft annotations.

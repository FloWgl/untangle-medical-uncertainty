...
def motion_blur(
    ...
    """
    if MotionImage is None:
        msg = "motion_blur requires wand and ImageMagick"
    ...
    x = cv2.imdecode(np.frombuffer(x.make_blob(), dtype=np.uint8), cv2.IMREAD_UNCHANGED)
...
def snow(
    ...
    """
    if MotionImage is None:
        msg = "snow requires wand and ImageMagick"
        raise RuntimeError(msg)
    ...
    snow_layer = (
        cv2.imdecode(
            np.frombuffer(snow_layer.make_blob(), dtype=np.uint8), cv2.IMREAD_UNCHANGED
        )
        / 255.0
    )

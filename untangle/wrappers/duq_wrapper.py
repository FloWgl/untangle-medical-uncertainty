...
class DUQHead(nn.Module):
    ...
    def forward(self, features: Tensor) -> Tensor | dict[str, Tensor]:
        ...
        Returns:
            ...
            during evaluation.
        """
        rbf_logits = self._rbf_logits(features)
        rbf_values = rbf_logits.exp()

        if self.training:
            return rbf_values
        # Normalizing exponentiated distances can produce 0 / 0 when every
        # class RBF underflows. Log-softmax is equivalent and remains finite.
        logit = F.log_softmax(rbf_logits, dim=1)
    ...
    def _rbf(self, features: Tensor) -> Tensor:
        ...
        Returns:
            RBF values.
        """
        return self._rbf_logits(features).exp()

    def _rbf_logits(self, features: Tensor) -> Tensor:
        """Calculates the unnormalized log RBF values."""
        latent_features = torch.einsum(
            "clf,bf->bcl", self._weight, features
        )  # [B, C, L]
        ...
        return diffs.square().mean(dim=-1).div(2 * self._rbf_length_scale**2).mul(-1)

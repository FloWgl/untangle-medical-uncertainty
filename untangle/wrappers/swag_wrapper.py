...
class SWAGWrapper(DistributionalWrapper):
    ...
    def _set_and_store_bn_stats(
        ...
        if fraction is not None:
            # Small-data loaders can contain fewer than ten batches. Always use
            # at least one batch so BatchNorm is not left in its reset state.
            num_batches = max(1, int(num_batches * fraction))

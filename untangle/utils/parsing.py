...
group.add_argument(
    ...
    type=Path,
    default=Path(),
    help="Path to soft-label root (used by soft datasets)",
)
group.add_argument(
...
group.add_argument(
    ...
    default="hard/pathmnist",
    help=(
        'ID eval + test dataset type + name ("<type>/<name>"), usually the same as '
        "--dataset for PathMNIST/PathMNIST-C or a soft-label variant when available"
    ),
)
group.add_argument(
    "--ood-transforms-eval",
...
group.add_argument(
    ...
    default=0.0,
    help=(
        "Fraction of training labels to replace by deterministic wrong labels. "
...
group.add_argument(
    ...
    default="symmetric",
    choices=("symmetric",),
    help="Training-label noise mode.",
)
group.add_argument(
    "--label-noise-seed",
    type=int,
    default=None,
    help="Seed for selecting and flipping noisy training labels. Defaults to --seed.",
)
group.add_argument(
...
group.add_argument(
    "--prior-precision-grid-size",
    type=int,
    default=500,
    help="Number of prior precision grid points used by Laplace",
)
group.add_argument(
    "--log-prior-precision-min",
    type=float,
    default=-1,
    help="Minimum log prior precision used by Laplace",
)
group.add_argument(
    "--log-prior-precision-max",
    type=float,
    default=2,
    help="Maximum log prior precision used by Laplace",
)
group.add_argument(
    "--use-low-rank-cov",
    action="store_true",
...
def parse_args() -> argparse.Namespace:
    ...
    # Backwards compatibility: some code references `soft_imagenet_label_dir`.
    # Provide an alias from the newer `soft_label_root` argument.
    if not hasattr(args, "soft_imagenet_label_dir"):
        args.soft_imagenet_label_dir = getattr(args, "soft_label_root", Path())
